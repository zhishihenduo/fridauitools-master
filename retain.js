...
function logi(msg) {
    common.log("JsHook-Bridge    " + msg)
    common.callStaticMethod("android.util.Log", "i", ["JsHook-Bridge", msg]);
}
...
function hookTTAd() {
    common.hookAllMethods('com.bytedance.sdk.openadsdk.TTAdConfig', 'getAppId', function (param) {
...
if (packageName == 'com.ss.android.ugc.aweme') {
    ...
    // TENativeLibsLoader["LIZIZ"].implementation = function () {
    //     logi(`TENativeLibsLoader.LIZIZ is called`);
    //     this["LIZIZ"]();
    // };
    ...
    //CCleaner本地解锁高级版
    hookAllMethods('com.avast.android.cleaner.subscription.ﹳ', '%u6e6', 1, true)
    ...
    common.hookAllMethods('android.app.ActivityThread', 'performLaunchActivity', null, function (param) {
        ...
        common.hookAllMethods(高级列表框Impl, '添加项目', function (param) {
            if (param.args[1].toString() == 'VIP到期时间') param.args[2] = '永久会员';
        });
    ...
    //搜图神器
    common.hookAllMethods('android.app.ActivityThread', 'performLaunchActivity', null, function (param) {
        var $class = classLoad(param.thisObject, '%u2c9.%u5d9.%u2be.%u2c8.%u640%u640.ᴵᴵ')
        hookAllMethods($class, '%u2bd', 1, true)
        hookAllMethods($class, '%u2bc', 1, true)
    });
} else if (packageName == 'com.qq.ssw12') {

    common.toast("ssw红包")

//var $class = classLoad(param.thisObject, ''')
    //hookAllMethods($class, '%u2bd', 1, true)

    hookAllMethods('java.lang.System', 'currentTimeMillis', 3, 166000000000)
